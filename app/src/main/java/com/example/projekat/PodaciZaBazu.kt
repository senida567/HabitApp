   ...
   // override suspend fun doWork(): Result = coroutineScope {
    ...
    suspend fun onCreate() {
        try {
            ...
            db.glavneAktivnostiDao().insert(GlavneAktivnosti(1, "Jutarnje trčanje", 1))
            ...
            db.glavneAktivnostiDao().insert(GlavneAktivnosti(3, "Računarske mreže", 3))
            db.inkrementalneDao().insert(Inkrementalne(1, 3, 1, 0))
            db.mjerneJediniceDao().insert(MjerneJedinice(3, "sati", 3))

            db.aktivnostiDao().insert(Aktivnosti(4, "Voda", 2))
